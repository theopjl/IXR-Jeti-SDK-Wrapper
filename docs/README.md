...
[![CI](https://github.com/yourusername/jeti-sdk-wrapper/actions/workflows/ci.yml/badge.svg)](https://github.com/yourusername/jeti-sdk-wrapper/actions/workflows/ci.yml)
...
| Script | Description |
|--------|-------------|
| `quick_start.py` | Simplest measurement example |
| `advanced_example.py` | Spectral analysis with data export |
| `radio_sample.py` | Basic radiometric measurement (from C sample) |
| `radio_sample_ex.py` | Extended radiometric measurements |
| `spectro_ex_sample.py` | Spectroscopic light measurements |
| `sync_sample.py` | Synchronized measurements with optical trigger |
...
| 0x00000008 | TIMEOUT | Timeout error |
| 0x00000014 | NOT_CONNECTED | Device not connected |
| 0x00000020 | OVEREXPOSURE | Sensor overexposure |
| 0x00000022 | MEASURE_FAIL | Measurement failed |
...
# Run with coverage
pytest tests/ --cov=jeti --cov-report=html
```
...
**"DLL not found"**
- Ensure all DLL files are in the `dlls/` directory
...
This wrapper is provided under the MIT License. See [LICENSE](LICENSE) for details.

The JETI SDK itself is proprietary software from JETI Technische Instrumente GmbH.
